...
class Envelope:
    ...
    def trap_ar(samples, ramp):
        ramp_data = Envelope.trap_ar_ramp_calc(len(samples), ramp)
    ...
    def adsr(samples, attack, decay, sustain, release):
        ramp_data = Envelope.adsr_ramp_calc(len(samples), attack=attack, decay=decay, sustain=sustain, release=release)
        final_data = (np.multiply(samples, ramp_data)).astype(np.int16)
        return final_data
